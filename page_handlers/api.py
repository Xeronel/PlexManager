...
class Unwatched(ApiBase):
    def get(self):
        ...
        cursor.execute(
            ...
            "UNION ALL "
            "SELECT M2.id, M2.title, M2.metadata_type, M2.guid, CTE.parent_name AS parent_name "
            "FROM metadata_items M2 "
            "JOIN CTE "
            "ON CTE.id = M2.parent_id) "
            "SELECT title, parent_name, show_type "
            "FROM CTE "
            "LEFT JOIN pdb_metadata_types "
            "ON pdb_metadata_types.id = CTE.metadata_type "
            "WHERE NOT EXISTS "
            "(SELECT CTE.guid "
            "FROM metadata_item_settings M1 "
            "WHERE M1.guid = CTE.guid) "
